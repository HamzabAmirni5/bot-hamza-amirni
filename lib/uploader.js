...
async function Tioxy(buffer) {
	try {
		...
		if (img.status && img.result && img.result.url) return img.result.url;
...
async function uploadImage(buffer) {
	try {
		// Try Tioxy first (Modern & High Speed)
		...
		try {
			...
			try {
				...
				console.error('All Uploaders Failed:', e3);
				throw e3;
			}
		}
	}
...
module.exports = { TelegraPh, UploadFileUgu, webp2mp4File, uploadImage, Catbox, Tioxy }