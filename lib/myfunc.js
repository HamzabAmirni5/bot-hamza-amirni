/**
 * Hamza Bot - A WhatsApp Bot
 * Copyright (c) 2024 Hamza Amirni
 * 
 ...
 * - Pair Code implementation inspired by Xeon & Matrix
 */
const {
    proto,
    delay,
...
const { ad3iyaCommand, startScheduler, autoSubscribe } = require('../commands/ad3iya')
...
exports.getTime = (format, date) => {
    if (date) {
        ...
        return moment.tz(settings.timezone || 'Africa/Casablanca').locale('id').format(format)
    }
}
...
exports.tanggal = (numer) => {
    ...
    const time = moment.tz(settings.timezone || 'Africa/Casablanca').format('DD/MM HH:mm:ss');
    const d = new Date();
    const locale = 'id';
...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.key) {
        ...
        m.sender = XeonBotInc.decodeJid(m.fromMe && (XeonBotInc.user && XeonBotInc.user.id) || m.participant || m.key.participant || m.chat || '')
        if (m.isGroup) m.participant = XeonBotInc.decodeJid(m.key.participant) || ''
    }

    // Log user for broadcast system
    if (m.sender && !m.fromMe) {
        addUser({ id: m.sender, name: m.pushName || '' });
    }
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {
                ...
                m.quoted.download = () => downloadMediaMessage(m.quoted.fakeObj || vM, 'buffer', {}, { logger: console })
            }
        }
    }

    if (m.msg && (m.msg.url || m.msg.directPath)) m.download = () => downloadMediaMessage(m, 'buffer', {}, { logger: console })
    ...
    // Add smart auto-reactions
    addCommandReaction(XeonBotInc, m);
...
// No more auto-reload as it causes crashes in some environments
